...
// var channel = fabric_client.newChannel('hibcchannel');
...
Fabric_Client.newDefaultKeyValueStore({ path: store_path
	...
	var request = {chaincodeId: 'hibccc',
		fcn: 'requestClaim',
		args: [req.body.policyId, req.body.carrierId, req.body.ailment],
		chainId: 'hibcchannel',
		txId: tx_id};
	...
	//	res.send({code:"200", message: "claim requested successsfully."});
	//} else {
...
function processClaim(req, res) {
	...
	Fabric_Client.newDefaultKeyValueStore({ path: store_path
		...
		var request = {chaincodeId: 'hibccc',
	fcn: 'processClaim',
	args: [req.body.claimId,req.body.trmnt_pkg_id,req.body.hc_id],
	chainId: 'hibcchannel',
	txId: tx_id};
		...
		//if(results && results[1] && results[1].event_status === 'VALID') {
		//	console.log('Successfully committed the change to the ledger by the peer');
		//	res.send({code:"200", message: "Claim processed successsfully."});
		//} else {
		//	console.log('Transaction failed to be committed to the ledger due to ::'+results[1].event_status);
		//}
	}).catch((err) => {
...
function approveClaim(req, res) {
	...
	txId: tx_id};
		...
		//if(results && results[1] && results[1].event_status === 'VALID') {
		//	console.log('Successfully committed the change to the ledger by the peer');
		//	res.send({code:"200", message: "Claim approved successsfully."});
		//} else {
		//	console.log('Transaction failed to be committed to the ledger due to ::'+results[1].event_status);
		//	res.send({code:"500", message: "Approve Claim failed."});
		//}
...
// Get current state of LC using Bank user
function getClaimHistory(req, res){
	...
	Fabric_Client.newDefaultKeyValueStore({ path: store_path
		...
		if (query_responses && query_responses.length == 1) {
			if (query_responses[0] instanceof Error) {
				console.error("error from query = ", query_responses[0]);
				res.send({code:"500", message: "Issue with getting Claim details"});
			} else {
			...
			console.log("No payloads were returned from query");
			res.send({code:"500", message: "No Claim found"});
		}
	}).catch((err) => {
		console.error('Failed to query successfully :: ' + err);
		res.send({code:"500", message: "Issue with getting Claim details"});
